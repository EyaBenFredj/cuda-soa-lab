...
async def train():
    ...
    logger.info("Starting local training on client3...")
    
    # Generate synthetic data for client3 (different seed)
    np.random.seed(44)
    ...
    logger.info(f"Client3 training completed: w={w_local:.4f}, b={b_local:.4f}")
    
    return {"w": float(w_local), "b": float(b_local)}

@app.get("/health")
async def health():
    return {"status": "healthy", "client": "client3"}
